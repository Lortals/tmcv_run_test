...
| RP | bd_pos (MSB,LSB)¹ | qp_1 | qp_2 | format_2 | 总几何精度 | QP_1 内部有效值² |
|----|------------------|------|------|----------|-----------|----------------|
| 1 | 10, 9 | **-8** | 0 | yuv444 | 19 位 | -8+12 = **4** |
| 2 | 10, 6 | **-4** | 4 | yuv444 | 16 位 | -4+12 = **8** |
| 3 | 9, 6 | 4 | 12 | yuv444 | 15 位 | 4+12 = **16** |
| 4 | 9, 6 | 8 | 16 | yuv420 | 15 位 | 8+12 = **20** |
| 5 | 8, 6 | 16 | 24 | yuv420 | 14 位 | 16+12 = **28** |

> ¹ `bd_pos (MSB,LSB)` 是 **Video 0**（几何坐标 x,y,z）的高低位精度，随码率点变化为 8/9/10 位；  
>   **Video 1**（几何属性：opacity / scale / rotation）的位深 `bd_1 = 10` 在所有码率点中**固定不变**。  
> ² 内部有效值 = qp_1 + QpBdOffset。Video 1 固定 10-bit，因此 QpBdOffset = 6×(10−8) = **12** 对所有 RP 均适用。  
>   负 QP 是 HM-RExt 对 10-bit 内容的合法扩展（合法范围 [−12, 51]），详见 `CTC_CONDITIONS.md §1.3`。  
>   MSB/LSB 机制详见 `CTC_CONDITIONS.md §六`。
